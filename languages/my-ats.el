...
;; (require 'ats-flymake)
;;
;; The ATSHOME environment variable must also be set from within emacs
;; because it doesn't seem to be automatically loaded from the environment.
;;
;; (setenv "ATSHOME" "/path/to/ats")

(require 'flymake)

(defvar ats-flymake-command
  "atscc"
  "Command used to check an ATS file for errors")

(defvar ats-flymake-command-options
  "-tc"
  "Options passed to the command used to check a file for errors")

(defun ats-flymake-init ()
  (let* ((temp-file   (flymake-init-create-temp-buffer-copy
		       'flymake-create-temp-inplace))
	 (local-file  (file-relative-name
		       temp-file
		       (file-name-directory buffer-file-name))))
    (list ats-flymake-command
	  (list ats-flymake-command-options local-file))))

;; List of file extensions that trigger ats-flymake.
(push '(".+\\.sats$" ats-flymake-init flymake-simple-cleanup) flymake-allowed-file-name-masks)
(push '(".+\\.dats$" ats-flymake-init flymake-simple-cleanup) flymake-allowed-file-name-masks)
(push '(".+\\.hats$" ats-flymake-init flymake-simple-cleanup) flymake-allowed-file-name-masks)
...
(add-hook 'ats-mode-hook 'ats-flymake-load)
